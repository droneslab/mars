...
def getCamImg(model, x):
    target_layers = [model.features.layer4[-1]]
    cam = EigenCAM(model=model, target_layers=target_layers, use_cuda=True)    
    cam = cam(input_tensor=x[None,:], aug_smooth=True, eigen_smooth=True)
    vis = show_cam_on_image(x.permute(1, 2, 0).cpu().numpy(), cam[0, :], use_rgb=True, image_weight=0.7, colormap=cv2.COLORMAP_JET)
    return vis
...
ours = sys.argv[2]
mname = sys.argv[3]
Path(f'../data/results/figs/cams/{mname}/').mkdir(parents=True, exist_ok=True)
...
for idx in idxs:
    ...
    scam1 = torch.tensor(getCamImg(stock_model, img1)).permute(2,0,1)
    scam2 = torch.tensor(getCamImg(stock_model, img2)).permute(2,0,1)
    scam1_norm = warp(scam1, t1, rot='+')
    scam2_norm = warp(scam2, t2, rot='+')
    
    ocam1 = torch.tensor(getCamImg(our_model, img1)).permute(2,0,1)
    ocam2 = torch.tensor(getCamImg(our_model, img2)).permute(2,0,1)
    ocam1_norm = warp(ocam1, t1, rot='+')
    ocam2_norm = warp(ocam2, t2, rot='+')
    
    fig,axs = plt.subplots(3,2, figsize=(4,5.4))
    fig.tight_layout()
    axs[0][0].imshow(img1.permute(1,2,0))
    axs[0][1].imshow(img2.permute(1,2,0))
    axs[1][0].imshow(scam1_norm.permute(1,2,0))
    axs[1][1].imshow(scam2_norm.permute(1,2,0))
    axs[2][0].imshow(ocam1_norm.permute(1,2,0))
    axs[2][1].imshow(ocam2_norm.permute(1,2,0))
    plt.setp(axs, xticks=[], yticks=[])
    plt.subplots_adjust(wspace=0.05, hspace=0.05)
    # plt.show()
    plt.savefig(f"../data/results/figs/cams/{mname}/{n}.png",
            bbox_inches ="tight",
            pad_inches = 0,
            transparent = True,
            dpi=800)

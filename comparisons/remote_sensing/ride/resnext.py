...
class ChannelPool(pl.LightningModule):
    def forward(self, x):
...
''' ======================================
        SENet154, SE-ResNet, SE-ResNeXt Models
    ====================================== '''
...
class SEBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.att = SEModule(planes * 4, reduction=reduction)
...
class CBAMResNeXtBottleneck(Bottleneck):
    """
    ResNeXt bottleneck type C with a Squeeze-and-Excitation module.
    """
    expansion = 4
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, base_width=4):
        super(CBAMResNeXtBottleneck, self).__init__()
        width = math.floor(planes * (base_width / 64)) * groups
        self.conv1 = nn.Conv2d(inplanes, width, kernel_size=1, bias=False,
                               stride=1)
        self.bn1 = nn.BatchNorm2d(width)
        ...
        self.relu = nn.ReLU(inplace=True)
        # self.att = SEModule(planes * 4, reduction=reduction)
        self.att = CBAM(planes * 4, reduction_ratio=reduction)
        self.downsample = downsample
        self.stride = stride
...
class Net(pl.LightningModule):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
        ...
        Parameters
        ----------
        block (pl.LightningModule): Bottleneck class.
...
''' ======================================
    ...
    ====================================== '''
def create_se_resnext101():
    model = Net(SEResNeXtBottleneck, [3, 4, 23, 3], groups=32, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0)
    return model

def create_cbam_resnext101():
    model = Net(CBAMResNeXtBottleneck, [3, 4, 23, 3], groups=32, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0)
    return model
...
class ResNext(pl.LightningModule):
    def __init__(self, att_type):
        super().__init__()
        self.bneck_type = att_type
        
        if att_type == 'se':
            self.resnext = create_se_resnext101()
        elif att_type == 'cbam':
            self.resnext = create_cbam_resnext101()
